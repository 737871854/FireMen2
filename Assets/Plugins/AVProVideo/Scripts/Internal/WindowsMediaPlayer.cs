...
namespace RenderHeads.Media.AVProVideo
{
	public class WindowsMediaPlayer : BaseMediaPlayer
	{
		...
		private System.IntPtr mInstance = System.IntPtr.Zero;
		...
		public override bool OpenVideoFromFile(string path)
		{
			...
			mInstance = Native.OpenSource(mInstance, path, _forceDirectShowApi, _audioDeviceOutputName);
		...
		{
			_bLoop = looping;
			Native.SetLooping(mInstance, looping);
		}
		...
		{
			return Native.IsSeeking(mInstance);
		}
		...
		{
			return Native.IsFinished(mInstance);
		}
		...
		{
			return Native.GetDuration(mInstance) * 1000f;
		}
		...
		{
			Texture result = null;
			int frameCount = Native.GetTextureFrameCount(mInstance);
		...
		{
			Texture result = null;
			if (Native.GetTextureFrameCount(mInstance) > 0)
			{
				result = _texture;
			}
			return result;
		...
		{
			return Native.GetTextureFrameCount(mInstance);
		}
		...
		{
			Native.SetCurrentTime(mInstance, timeMs / 1000f, false);
		}

		public override void SeekFast(float timeMs)
		{
			Native.SetCurrentTime(mInstance, timeMs / 1000f, true);
		}

		public override float GetCurrentTimeMs()
		{
			return Native.GetCurrentTime(mInstance) * 1000f;
		}

		public override void SetPlaybackRate(float rate)
		{
			Native.SetPlaybackRate(mInstance, rate);
		}

		public override float GetPlaybackRate()
		{
			return Native.GetPlaybackRate(mInstance);
		}

		public override void MuteAudio(bool bMuted)
		{
			_audioMuted = bMuted;
			Native.SetMuted(mInstance, _audioMuted);
		}
		...
		{
			_volume = volume;
			Native.SetVolume(mInstance, volume);
		}
		...
		{
			Native.Update(mInstance);
			_lastError = (ErrorCode)Native.GetLastErrorCode(mInstance);

			if (!_canPlay)
			{
				if (!_hasMetaData)
				{
					if (Native.HasMetaData(mInstance))
					{
						if (Native.HasVideo(mInstance))
						{
							_width = Native.GetWidth(mInstance);
							_height = Native.GetHeight(mInstance);
						...
						string playerDescription = System.Runtime.InteropServices.Marshal.PtrToStringAnsi(Native.GetPlayerDescription(mInstance));
						Debug.Log("[AVProVideo] Using player type " + playerDescription);
					}
				}
				if (_hasMetaData)
				{
					_canPlay = Native.CanPlay(mInstance);
				}
			}
			...
			if (_hasVideo && _width > 0 && _height > 0)
			{
				...
				if (_textures == null)
				{
					System.IntPtr ptr = Native.GetTexturePointers(mInstance, 0);
					if (ptr != System.IntPtr.Zero)
					{
						_textures = new Texture2D[NUM_TEXTURE_FRAMES];
						for (int i = 0; i < _textures.Length; i++)
						{
							ptr = Native.GetTexturePointers(mInstance, i);
				...
				if (_texture == null)
				{
					System.IntPtr ptr = Native.GetTexturePointer(mInstance);
					if (ptr != System.IntPtr.Zero)
					{
						_isTextureTopDown = Native.IsTextureTopDown(mInstance);
